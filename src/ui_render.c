...
void ui_draw_circuit(AppContext *app, Rectangle canvas) {
    ...
    for (i = 0; i < graph->net_count; i++) {
        ...
        net = &graph->nets[i];
        if (!net->source || net->source->node->type == (NodeType)-1) {
            continue;
        }
        ...
        for (sink_index = 0; sink_index < net->sink_count; sink_index++) {
            ...
            if (sink_pin->node->type == (NodeType)-1) {
                continue;
            }
...
void ui_draw_waveforms(AppContext *app, Rectangle panel) {
    ...
    if (total_rows > (int)visible_rows) {
        hidden_rows = (uint32_t)(total_rows - (int)visible_rows);
        snprintf(overflow_text, sizeof(overflow_text), "+%u more", hidden_rows);
        draw_text_at(
            overflow_text,
